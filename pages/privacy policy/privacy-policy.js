...
(function ItemPageModule() {
  ...
  (function QuantityControls() {
    ...
    function onPlusClick(e) {
      e && e.preventDefault();
      let current = readQty();
      if (current < MAX_QTY_PER_ITEM) writeQty(current + 1);
      else StoreApp.showToast(`⚠ Max per item: ${MAX_QTY_PER_ITEM}`, "main-toast");
    }

    function onMinusClick(e) {
      e && e.preventDefault();
      let current = readQty();
      if (current > MIN_QTY) writeQty(current - 1);
      else StoreApp.showToast(`⚠ Min per item: ${MIN_QTY}`, "main-toast");
    }
